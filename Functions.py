#Functions library. Main.py imports functions from this file
import numpy as np
import os
...
def calculate_cross_correlation(t1, t_actual):
    ...
    part_length = min(t1_length, t_actual_length)
    length = []
    length_value = []

    for i in range(0, t_actual_length, part_length):
        t1_part = t1[:part_length]  
      ...
      #Normalisation is not required in this data  
        ...
        epsilon = 1e+8  #small constant for normalisation
        t_actual_norm = (t_actual_part - np.mean(t_actual_part)) / (np.std(t_actual_part)+ epsilon)
      #autocorrelation
    ...
    #%age
        ...
        phase_differences.append(phase_difference)
         #phase block creations
    '''    if 79 <= cross_correlation_percentage <= 100:
            phase_difference = 0
            phase_differences.append(phase_difference)
        elif 39 <= cross_correlation_percentage <= 80:
            phase_difference = 90
            phase_differences.append(phase_difference)
        elif -39 <= cross_correlation_percentage <= -80:
            phase_difference = -90
